...
from uber.models import AutomatedEmail, Email, MagModel, UberSession, Session, async_session
...
async def notify_admins_of_pending_emails():
    """
...
@task
async def check_emails_for_fixture(id):
...
async def send_automated_emails():
    """
    Send any queued emails concurrently while using DB locks to ensure the same email doesn't get processed twice.
    Emails are processed in parallel per model.
    """
                ...
                with Session.engine.connect() as guard_conn:
                    ...
                    with guard_conn.begin():
                        if guard_conn.execute(select(func.pg_try_advisory_lock(lock_key))).scalar():
                            ...
                            quantity_sent += await EmailService.process_emails_by_class_async(session, model_class)
