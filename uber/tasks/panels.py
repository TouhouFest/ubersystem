...
from uber.models import Email, async_session, Tracking, Department, EventLocation, AutomatedEmail
...
@task
async def sync_guidebook_models(selected_model, sync_time, id_list):
...
async def check_deleted_guidebook_models():
...
async def check_stale_guidebook_models():
...
async def panels_waitlist_unaccepted_panels():
...
async def setup_panel_emails(reconcile_fixtures=True):
    if not c.PRE_CON:
        return
