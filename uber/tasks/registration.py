...
from uber.models import (ApiJob, Attendee, AttendeeAccount, BadgeInfo, BadgePickupGroup, Email, Group, ModelReceipt,
                         ReceiptInfo, ReceiptItem, ReceiptTransaction, Session, async_session, TerminalSettlement)
from uber.tasks import crontab, schedule, task
...
async def create_badge_nums():
    ...
    async with async_session() as session:
        ...
        await session.execute(insert(BadgeInfo), new_badge_list)
        await session.commit()


@task
async def update_receipt(attendee_id, params):
    async with async_session() as session:
...
async def check_duplicate_registrations():
    """
...
@schedule(crontab(minute=0, hour='*/6'))
async def check_placeholder_registrations():
    if c.PRE_CON and (c.DEV_BOX or c.SEND_EMAILS) and c.REPORTS_EMAIL:
        emails = [[
            'Staff',
            c.STAFF_EMAIL,
...
@schedule(crontab(minute=0, hour='*/6'))
async def check_pending_badges():
    if c.PRE_CON and (c.DEV_BOX or c.SEND_EMAILS) and c.REPORTS_EMAIL:
...
@schedule(crontab(minute=0, hour='*/6'))
async def check_unassigned_volunteers():
    if c.PRE_CON and (c.DEV_BOX or c.SEND_EMAILS) and c.REPORTS_EMAIL:
        async with async_session() as session:
...
@schedule(timedelta(days=1))
async def invalidate_at_door_badges():
    if not c.POST_CON:
        return
...
@schedule(timedelta(days=1))
async def invalidate_dealer_badges():
...
@schedule(timedelta(days=1))
async def email_pending_attendees():
...
@task
async def send_receipt_email(receipt_id):
    async with async_session() as session:
...
@task
async def close_out_terminals(workstation_and_terminal_ids, who):
...
@task
async def process_terminal_sale(workstation_num, terminal_id, model_id=None, pickup_group_id=None, **kwargs):
    from uber.payments import SpinTerminalRequest
...
async def check_missed_stripe_payments():
    if c.AUTHORIZENET_LOGIN_ID:
        return
    ...
    stripe_limiter = asyncio.Semaphore(10)

    events = await stripe.Event.list_async(type='payment_intent.succeeded', created={
        'gte': int(time.time() - 60 * 60),
    })

    async def process_intent(payment_intent):
        if payment_intent.id in pending_ids:
            async with stripe_limiter:
                await asyncio.to_thread(ReceiptManager.mark_paid_from_stripe_intent, payment_intent)
                return payment_intent.id
        return None

    tasks = [process_intent(event.data.object) for event in events.auto_paging_iter()]
    results = await asyncio.gather(*tasks, return_exceptions=True)
    paid_ids = [res for res in results if res and isinstance(res, str)]
    return paid_ids


@schedule(timedelta(hours=3))
async def check_authnet_held_txns():
    if not c.AUTHORIZENET_LOGIN_ID:
        return

    held_ids = []
    
    def fetch_unsettled_list():
        merchantAuth = apicontractsv1.merchantAuthenticationType(
            name=c.AUTHORIZENET_LOGIN_ID,
            transactionKey=c.AUTHORIZENET_LOGIN_KEY
        )
        heldReq = apicontractsv1.getUnsettledTransactionListRequest()
        heldReq.merchantAuthentication = merchantAuth
        heldReq.status = 'pendingApproval'
        ctrl = apicontrollers.getUnsettledTransactionListController(heldReq)
        ctrl.execute()
        return ctrl.getresponse()

    heldTransactionListResponse = await asyncio.to_thread(fetch_unsettled_list)
    ...
    authnet_limiter = asyncio.Semaphore(10)
    ...
    async with async_session() as session:
        ...
        async def check_single_authnet_charge(charge_id, txns):
            async with authnet_limiter:
                txn_status = TransactionRequest()
                error = await asyncio.to_thread(txn_status.get_authorizenet_txn, charge_id)
                return charge_id, txns, txn_status, error

        tasks = [check_single_authnet_charge(cid, txs) for cid, txs in release_txns_by_charge_id.items()]
        results = await asyncio.gather(*tasks, return_exceptions=True)

        for res in results:
            if isinstance(res, Exception) or not res:
                continue
            charge_id, txns, txn_status, error = res
            if error:
...
@schedule(timedelta(days=1))
async def create_badge_pickup_groups():
...
async def reassign_purchaser_ids():
    async with async_session() as session:
...
async def sunset_empty_accounts():
    async with async_session() as session:
...
@task
async def import_attendee_accounts(accounts, admin_id, admin_name, target_server, api_token):
    already_queued = 0
    async with async_session() as session:
