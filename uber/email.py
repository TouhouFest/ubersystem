...
class EmailService:
    ...
    def process_emails_by_class(session, model_class):
        ...
        if loop and loop.is_running():
            return asyncio.create_task(EmailService.process_emails_by_class_async(session, model_class))
        else:
            return asyncio.run(EmailService.process_emails_by_class_async(session, model_class))

    @staticmethod
    async def process_emails_by_class_async(session, model_class):
        ...
        # Parallelize email dispatches concurrently across the network!
        tasks = [
            EmailService.send_email_async(session, email, email.automated_email, models_by_id.get(email.fk_id, None))
            for email in queued_emails
        ]
        results = await asyncio.gather(*tasks, return_exceptions=True)

        sent_count = 0
        for sent_email in results:
            if sent_email and isinstance(sent_email, Email):
    ...
    def send_email(session, email, fixture_obj=None, to_model=None):
        ...
        if loop and loop.is_running():
            return asyncio.create_task(EmailService.send_email_async(session, email, fixture_obj, to_model))
        else:
            return asyncio.run(EmailService.send_email_async(session, email, fixture_obj, to_model))

    @staticmethod
    async def send_email_async(session, email, fixture_obj=None, to_model=None):
        ...
        try:
            ...
            if not c.DEV_BOX and c.SEND_EMAILS:
                ...
                error_msg = await email_sender.send_email_async(
                    source=email.sender,
