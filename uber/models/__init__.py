...
from sqlalchemy.dialects.sqlite.base import SQLiteCompiler, SQLiteDDLCompiler
SQLiteCompiler.visit_sequence = lambda self, seq, **kw: "1"
SQLiteDDLCompiler.visit_create_sequence = lambda self, create, **kw: ""
SQLiteDDLCompiler.visit_drop_sequence = lambda self, drop, **kw: ""
...
class UberSession(sqlalchemy.orm.Session):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for name, val in self.SessionMixin.__dict__.items():
            if not name.startswith('__') and hasattr(val, '__call__') and name not in self.__dict__:
                setattr(self, name, MethodType(val, self))

    def __getattr__(self, name):
        if hasattr(self.SessionMixin, name):
            val = getattr(self.SessionMixin, name)
...
class HybridSessionProxy:
    ...
    """
    def __setattr__(self, name, value):
        if name == 'session_factory' and hasattr(value, 'class_'):
            value.class_ = UberSession
            if hasattr(value, 'kw') and 'class_' in value.kw:
                value.kw.pop('class_', None)
            _ScopedSession.session_factory = value
        super().__setattr__(name, value)
    ...
    def __getattr__(self, name):
        ...
        try:
            ...
            return getattr(UberSession, name)
    ...
    def __call__(self, create_savepoint=False, *args, **kwargs):
        ...
        """
        factory = getattr(self, 'session_factory', SessionFactory)
        try:
            req = cherrypy.request
            
            if 'bind' not in kwargs:
                if not hasattr(req, 'db_connection'):
                    req.db_connection = getattr(self, 'engine', engine).connect()
...
def initialize_db(drop=False, modify_tables=False, *args, **kwargs):
    """Explicitly create tables."""
    log.info("Initializing DB")
    bind_engine = getattr(Session, 'engine', engine)
    if drop and bind_engine:
        SQLModel.metadata.drop_all(bind=bind_engine)
    if bind_engine:
        SQLModel.metadata.create_all(bind=bind_engine)
...
_ScopedSession.initialize_db = initialize_db
