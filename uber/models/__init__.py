...
class HybridSessionProxy:
    ...
    async def __aenter__(self):
        ...
        return AsyncUberSession(session)

    async def __aexit__(self, exc_type, exc_val, exc_tb):
        pass


class AsyncUberSession:
    """
    Asynchronous session wrapper for SQLAlchemy in SAQ background tasks.
    Supports `async with async_session() as session:` or `async with Session() as session:`.
    Dispatches blocking database calls asynchronously without blocking the asyncio event loop.
    """
    ...
    def _async_or_sync(self, fn, *args, **kwargs):
        try:
            import asyncio
    ...
    def commit(self):
        return self._async_or_sync(self._session.commit)

    def flush(self):
        return self._async_or_sync(self._session.flush)

    def rollback(self):
        return self._async_or_sync(self._session.rollback)

    def close(self):
        return self._async_or_sync(self._session.close)

    def execute(self, *args, **kwargs):
        return self._async_or_sync(self._session.execute, *args, **kwargs)

    def scalars(self, *args, **kwargs):
        return self._async_or_sync(self._session.scalars, *args, **kwargs)
...
def async_session(create_savepoint=False, *args, **kwargs):
    """Creates an asynchronous session context for background tasks."""
    return AsyncUberSession(Session(create_savepoint=create_savepoint, *args, **kwargs))


AsyncSession = AsyncUberSession
