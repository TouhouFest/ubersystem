    ...
    def __init__(
        ...
        for filter_tuple in [('filter', filter), ('send_filter', send_filter)]:
            ...
            if val and callable(val):
                setattr(self, name, lambda x, fn=val: bool(x.gets_emails and fn(x)))
                try:
                    setattr(self, name + '_desc', inspect.getsource(val).strip())
                except Exception:
                    setattr(self, name + '_desc', getattr(val, '__name__', str(val)))
...
class MarketplaceEmailFixture(AutomatedEmailFixture):
    def __init__(self, subject, template, filter, ident, **kwargs):
        dealer_filter = (lambda g, f=filter: bool(g.is_dealer and f(g))) if (filter and callable(filter)) else (lambda g: g.is_dealer)
...
class StopsEmailFixture(AutomatedEmailFixture):
    def __init__(self, subject, template, filter, ident, **kwargs):
        staff_filter = (lambda a, f=filter: bool(a.staffing and f(a))) if (filter and callable(filter)) else (lambda a: a.staffing)
...
class HotelLotteryEmailFixture(AutomatedEmailFixture):
    def __init__(self, subject, template, filter, ident, **kwargs):
        lottery_filter = (lambda a, f=filter: bool(a.attendee and f(a))) if (filter and callable(filter)) else (lambda a: a.attendee)
...
class RetroGuestEmailFixture(AutomatedEmailFixture):
    def __init__(self, subject, template, filter, ident, **kwargs):
        guest_filter = (lambda mg, f=filter: bool(mg.group_type == c.MIVS and mg.group.studio and mg.matches_showcases([c.INDIE_RETRO]) and f(mg))) if (filter and callable(filter)) else (lambda mg: bool(mg.group_type == c.MIVS and mg.group.studio and mg.matches_showcases([c.INDIE_RETRO])))
...
class IAGuestEmailFixture(AutomatedEmailFixture):
    def __init__(self, subject, template, filter, ident, **kwargs):
        guest_filter = (lambda mg, f=filter: bool(mg.group_type == c.MIVS and mg.group.studio and mg.matches_showcases([c.INDIE_ARCADE]) and f(mg))) if (filter and callable(filter)) else (lambda mg: bool(mg.group_type == c.MIVS and mg.group.studio and mg.matches_showcases([c.INDIE_ARCADE])))
...
class MIVSGuestEmailFixture(AutomatedEmailFixture):
    def __init__(self, subject, template, filter, ident, **kwargs):
        guest_filter = (lambda mg, f=filter: bool(mg.group_type == c.MIVS and mg.group.studio and mg.matches_showcases([c.MIVS]) and f(mg))) if (filter and callable(filter)) else (lambda mg: bool(mg.group_type == c.MIVS and mg.group.studio and mg.matches_showcases([c.MIVS])))
...
class PanelAppEmailFixture(AutomatedEmailFixture):
    def __init__(self, subject, template, filter, ident, **kwargs):
        panel_filter = (lambda a, f=filter: bool((not a.submitter or not a.submitter.attendee_id) and f(a))) if (filter and callable(filter)) else (lambda a: bool(not a.submitter or not a.submitter.attendee_id))
...
class ArenaEmailFixture(AutomatedEmailFixture):
    def __init__(self, subject, template, filter, ident, **kwargs):
        arena_filter = (lambda g, f=filter: bool(g.group_type == c.ARENA and f(g))) if (filter and callable(filter)) else (lambda g: g.group_type == c.ARENA)
...
class BandEmailFixture(AutomatedEmailFixture):
    def __init__(self, subject, template, filter, ident, **kwargs):
        band_filter = (lambda g, f=filter: bool(g.group_type in [c.BAND, c.SIDE_STAGE] and f(g))) if (filter and callable(filter)) else (lambda g: g.group_type in [c.BAND, c.SIDE_STAGE])
...
class GuestEmailFixture(AutomatedEmailFixture):
    def __init__(self, subject, template, filter, ident, **kwargs):
        guest_filter = (lambda g, f=filter: bool(g.group_type == c.GUEST and f(g))) if (filter and callable(filter)) else (lambda g: g.group_type == c.GUEST)
