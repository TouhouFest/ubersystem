...
WORKDIR /app
ENV PYTHONPATH=/app
ENV PATH=/app/.venv/bin:/root/.local/bin:/usr/local/bin:/usr/local/sbin:/usr/sbin:/usr/bin:/sbin:/bin
...
# Alternatively, build lxml from source to link against system libxml2: RUN uv pip install --no-binary lxml lxml
...
RUN uv pip install https://github.com/magfest/lxml/releases/download/v$LXML/lxml-$LXML-cp313-cp313-musllinux_1_2_$(uname -m).whl
